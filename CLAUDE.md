...
| `netlify/functions/pulse-fetch.mjs` | Per-story "Opinion on this story" endpoint (World/India): GET = enabled probe, POST ?section=&id= runs `fetchPulseForItem` (pulse.mjs) for that one story and persists the result into the blob. Every eligible story shows the button when enabled; first click fetches live, after that it folds. No budget ledger — Bluesky limits are generous. |
...
| `src/pulse.mjs` + `src/pulseConfig.mjs` + `src/sentimentLexicon.mjs` | Public Pulse: reader reaction on World/India ONLY — enrichment runs in refresh-news.mjs after scoring, before the blob write; Tech/Esports untouched. `item.pulse` is an ARRAY of provider entries: Bluesky (`BLUESKY_HANDLE`+`BLUESKY_APP_PASSWORD`, app password not the login), YouTube comments (`YOUTUBE_API_KEY` — search.list has its own ~100/day bucket since Jun 2026, hence yt perDeskCap 4 vs bluesky 8 and stored-video-ID reuse; commentsDisabled = normal no-match, try next candidate), optional Mastodon (World only, `MASTODON_ENABLED=true`). AFINN-165 lexicon shared by all providers; `pulse: []` on no match. **Tuning knobs live in pulseConfig.mjs.** |
| `src/opinion.mjs` | Opinion/editorial detection (URL path segments, RSS `<category>` tags, byline patterns — no model). **World + India ONLY** (opt-in via `def.opinion` in the builders; Tech/Esports never flagged). Flagged items get `contentType:"opinion"`, never enter dedupe/scoring/corroboration; a topical title match also pins them as `story.opinions[]` (per-story OPINION button), and all of them list behind the desk's OPINION flip view. Guards in finish(), mergeManualDiscovery, attachCommentary, pulse targets. The same per-story button also unfolds `pulse.voices` — real Bluesky/Mastodon replies split "Readers for"/"Readers against" by AFINN tone sign (`buildVoices` in pulse.mjs, `voicesPerSide` in pulseConfig), always with the not-a-stance-classifier disclosure. |
